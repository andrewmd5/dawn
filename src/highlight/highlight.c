...
static compiled_pattern_t compile_pattern(const char *pattern, uint32_t flags, hl_ctx_t *ctx) {
    ...
    if (flags & HL_RULE_MULTILINE) options |= PCRE2_MULTILINE;
    if (flags & HL_RULE_CASELESS) options |= PCRE2_CASELESS;
    if (flags & HL_RULE_DOTALL) options |= PCRE2_DOTALL;
    ...
    while (pos < end) {
        ...
        for (size_t i = 0; i < lang->pattern_count; i++) {
            ...
            if (cache[i].valid && cache[i].start >= pos) {
                if (cache[i].start < best_start ||
                    (cache[i].start == best_start && cache[i].end > best_end)) {
...
int32_t hl_tokenize(hl_ctx_t *ctx,
    ...
    if (!ctx || !code || !callback) {
        if (ctx) ctx_set_error(ctx, "Invalid arguments to hl_tokenize");
        return -1;
    }
...
char *hl_highlight_ex(hl_ctx_t *ctx,
                      ...
                      size_t *out_len) {
    if (!ctx || !code) {
        if (ctx) ctx_set_error(ctx, "Invalid arguments to hl_highlight_ex");
        return NULL;
    }
...
const char *hl_ctx_detect_language(hl_ctx_t *ctx, const char *code, size_t code_len) {
    if (!ctx || !code || code_len == 0) return "plain";
    ...
    for (lang_entry_t *e = ctx->languages; e; e = e->next) {
        ...
        if (!lang || !lang->detect || lang->detect_count == 0) continue;
