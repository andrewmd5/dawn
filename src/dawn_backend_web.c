...
EM_JS(void, js_draw_text_scaled, (int32_t col, int32_t row, const char* text, int32_t scale, int32_t fg_r, int32_t fg_g, int32_t fg_b, int32_t bg_r, int32_t bg_g, int32_t bg_b, int32_t bold, int32_t italic, int32_t dim), {
    const ctx = window.dawnCtx;
...
EM_JS(void, js_draw_text_scaled_frac, (int32_t col, int32_t row, const char* text, int32_t cell_scale, double font_scale, int32_t fg_r, int32_t fg_g, int32_t fg_b, int32_t bg_r, int32_t bg_g, int32_t bg_b, int32_t bold, int32_t italic, int32_t dim), {
    const ctx = window.dawnCtx;
    const str = UTF8ToString(text);
    const x = (col - 1) * window.dawnCellWidth;
    const y = (row - 1) * window.dawnCellHeight;
...
EM_JS(void, js_draw_text, (int32_t col, int32_t row, const char* text, int32_t num_cols, int32_t fg_r, int32_t fg_g, int32_t fg_b, int32_t bg_r, int32_t bg_g, int32_t bg_b, int32_t bold, int32_t italic, int32_t dim, int32_t underline, int32_t strikethrough), {
    const ctx = window.dawnCtx;
    const str = UTF8ToString(text);
    const x = (col - 1) * window.dawnCellWidth;
    const y = (row - 1) * window.dawnCellHeight;
...
EM_JS(void, js_get_local_time, (int32_t* year, int32_t* mon, int32_t* mday, int32_t* hour, int32_t* min, int32_t* sec, int32_t* wday), {
    const d = new Date();
...
static bool web_init(DawnMode mode)
{
    ...
    web_state.capabilities = DAWN_CAP_TRUE_COLOR | DAWN_CAP_SYNC_OUTPUT | DAWN_CAP_STYLED_UNDERLINE | DAWN_CAP_TEXT_SIZING | DAWN_CAP_IMAGES | DAWN_CAP_MOUSE | DAWN_CAP_BRACKETED_PASTE | DAWN_CAP_CLIPBOARD;
...
static void web_get_size(int32_t* out_cols, int32_t* out_rows)
{
    js_get_size(&web_state.cols, &web_state.rows);
    if (out_cols)
        *out_cols = web_state.cols;
    if (out_rows)
        *out_rows = web_state.rows;
}
...
static void parse_ansi_sgr(const char* params, size_t len)
{
    ...
    for (int32_t i = 0; i < num_count; i++) {
        ...
        case 2:
            ...
            break;
        case 3:
            web_state.italic = true;
            break;
        case 4:
            web_state.underline = true;
            break;
        case 9:
            web_state.strikethrough = true;
            break;
        case 22:
            web_state.bold = false;
            web_state.dim = false;
...
static bool web_image_is_supported(const char* path)
{
    ...
    if (!ext)
        return false;
    ext++;
    return (strcasecmp(ext, "png") == 0 || strcasecmp(ext, "jpg") == 0 || strcasecmp(ext, "jpeg") == 0 || strcasecmp(ext, "gif") == 0 || strcasecmp(ext, "bmp") == 0 || strcasecmp(ext, "webp") == 0);
}
