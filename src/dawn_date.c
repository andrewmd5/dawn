...
{
    return ((2 * (3 - (yr / 100) % 4)) + (yr % 100) + ((yr % 100) / 4) + (isleap(yr) ? 6 : 0) + 1) % 7;
}
...
#define TIME_MAX INT64_MAX
    ...
    if (!HAVE_YDAY(state) && HAVE_YEAR(state)) {
        if (HAVE_MON(state) && HAVE_MDAY(state)) {
            tm->tm_yday = start_of_month[isleap_sum(tm->tm_year, TM_YEAR_BASE)][tm->tm_mon] + (tm->tm_mday - 1);
            state |= S_YDAY;
            ...
            tm->tm_yday = (7 - first_wday_of(tm->tm_year + TM_YEAR_BASE) + day_offset) % 7 + (week_offset - 1) * 7 + tm->tm_wday - day_offset;
            state |= S_YDAY;
        }
    }
    ...
    if (HAVE_YDAY(state) && HAVE_YEAR(state)) {
        ...
        if (!HAVE_MON(state)) {
            ...
            while (tm->tm_yday >= start_of_month[isleap_val][i])
                i++;
            if (i > 12) {
                i = 1;
